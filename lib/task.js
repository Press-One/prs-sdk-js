 ...
 * @returns {Promise}
 */
Task.prototype.list = function ({ projectId, filter, type, offset, limit}, authOpts) {
  validator.assert(authOpts, 'authOpts cannot be null');
  validator.assert(authOpts.privateKey || authOpts.token, 'authOpts.privateKey or authOpts.token cannot be null');
 ...
 * @returns {Promise}
 */
Task.prototype.get = function (projectId, taskId, authOpts) {

  validator.assert(projectId, 'projectId cannot be null');
  validator.assert(taskId, 'taskId cannot be null');
  let query = { projectId };
  return request({
    host: this.config.getHost(),
    method: 'get',
