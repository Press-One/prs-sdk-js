...
describe('auth', function () {
  it('create new dapp', async function () {
    try {
      const privateKey = utility.recoverPrivateKey(developer.keystore, developer.password);
      const res = await P1.createApp(privateKey, {
        name: 'Test APP ' + new Date(),
  ...
  it('create new keypair', function () {
    newKeyPair = P1.createKeyPair();
    assert.ok(newKeyPair, 'create keypair error');
  });
...
describe('sign file and create contract', function () {
  it('create new keypair', function () {
    newKeyPair = P1.createKeyPair();
    assert.ok(newKeyPair, 'create keypair error');
  });
  ...
  it('sign text/markdown file', async function () {
    ...
    try {
      ...
      const res = await P1.signFile(newKeyPair.privateKey, {
        file: content,
        filename: 'xxx.md',
        title: 'xxx'
      });
  ...
  it('get contract templates', async function () {
    try {
      const res = await P1.getContractTemplates(newKeyPair.privateKey);
      console.log(JSON.stringify(res.body));
      res.status.should.equal(200);
    } catch (err) {
      assert.fail(JSON.stringify(err && err.response && err.response.body));
  ...
  it('create contract', async function () {
    ...
    License usage2 CNB:0.002 Terms: 这是商业使用条款，允许\\n修改和复制。`;
      const res = await P1.createContract(newKeyPair.privateKey, code);
      console.log(JSON.stringify(res.body));
      res.status.should.equal(200);
  ...
  it('bind contract to file', async function () {
    try {
      const res = await P1.bindContract(newKeyPair.privateKey, fileId, fileHash, contractId);
      console.log(JSON.stringify(res.body));
      res.status.should.equal(200);
    } catch (err) {
      assert.fail(JSON.stringify(err && err.response && err.response.body));
  ...
  it('get contract orders for seller', async function () {
    try {
      const res = await P1.getContractOrders(newKeyPair.privateKey, contractId);
      console.log(JSON.stringify(res.body));
      res.status.should.equal(200);
    } catch (err) {
      assert.fail(JSON.stringify(err && err.response && err.response.body));
  ...
  it('get contract by rId for seller', async function () {
    try {
      const res = await P1.contractInfo(newKeyPair.privateKey, contractId);
      console.log(JSON.stringify(res.body));
      res.status.should.equal(200);
    } catch (err) {
      assert.fail(JSON.stringify(err && err.response && err.response.body));
  ...
  it('list contracts by seller', async function () {
    try {
      const res = await P1.myContracts(newKeyPair.privateKey);
      console.log(JSON.stringify(res.body));
      res.status.should.equal(200);
    } catch (err) {
      assert.fail(JSON.stringify(err && err.response && err.response.body));
  ...
  it('list contracts by file msghash for seller', async function () {
    try {
      const res = await P1.contractByFileHash(newKeyPair.privateKey, fileHash);
      console.log(JSON.stringify(res.body));
      res.status.should.equal(200);
    } catch (err) {
      assert.fail(JSON.stringify(err && err.response && err.response.body));
